...
static void _freeMicroservice(natsMicroservice *m);
static void _retainMicroservice(natsMicroservice *m);
static void _releaseMicroservice(natsMicroservice *m);
...
static natsStatus
_respond(natsMicroservice *m, natsMicroserviceRequest *r, const char *data, int len);
...
_handleMicroservicePing(natsConnection *nc, natsSubscription *sub, natsMsg *msg, void *closure)
{
    ...
    if (s == NATS_OK)
    {
        s = _respond(m, &req, natsBuf_Data(buf), natsBuf_Len(buf));
    }
    if (buf != NULL)
    {
        natsBuf_Destroy(buf);
...
_createMicroservice(natsMicroservice **new_microservice, natsConnection *nc, natsMicroserviceConfig *cfg)
{
    ...
    if (s == NATS_OK)
    {
        s = _addVerbHandlers(m, natsMicroserviceVerbPing, _handleMicroservicePing);
    }
    if (s == NATS_OK)
    {
        *new_microservice = m;
